...
NESEmulator.PPU = {}
NESEmulator.PPU._mt = {__index = NESEmulator.PPU}
function NESEmulator.PPU:new(conf, cpu, palette)
    local ppu = {}
    setmetatable(ppu, NESEmulator.PPU._mt)
    ppu:initialize(conf, cpu, palette)
    return ppu
end

NESEmulator.PPU.SCREEN_HEIGHT = 240
NESEmulator.PPU.SCREEN_WIDTH = 256
-- clock/timing constants (stolen from Nestopia)
NESEmulator.PPU.RP2C02_CC = 4
NESEmulator.PPU.RP2C02_HACTIVE = NESEmulator.PPU.RP2C02_CC * 256
NESEmulator.PPU.RP2C02_HBLANK = NESEmulator.PPU.RP2C02_CC * 85
NESEmulator.PPU.RP2C02_HSYNC = NESEmulator.PPU.RP2C02_HACTIVE + NESEmulator.PPU.RP2C02_HBLANK
NESEmulator.PPU.RP2C02_VACTIVE = 240
NESEmulator.PPU.RP2C02_VSLEEP = 1
NESEmulator.PPU.RP2C02_VINT = 20
NESEmulator.PPU.RP2C02_VDUMMY = 1
NESEmulator.PPU.RP2C02_VBLANK = NESEmulator.PPU.RP2C02_VSLEEP + NESEmulator.PPU.RP2C02_VINT + NESEmulator.PPU.RP2C02_VDUMMY
NESEmulator.PPU.RP2C02_VSYNC = NESEmulator.PPU.RP2C02_VACTIVE + NESEmulator.PPU.RP2C02_VBLANK
NESEmulator.PPU.RP2C02_HVSYNCBOOT = NESEmulator.PPU.RP2C02_VACTIVE * NESEmulator.PPU.RP2C02_HSYNC + NESEmulator.PPU.RP2C02_CC * 312
NESEmulator.PPU.RP2C02_HVINT = NESEmulator.PPU.RP2C02_VINT * NESEmulator.PPU.RP2C02_HSYNC
NESEmulator.PPU.RP2C02_HVSYNC_0 = NESEmulator.PPU.RP2C02_VSYNC * NESEmulator.PPU.RP2C02_HSYNC
NESEmulator.PPU.RP2C02_HVSYNC_1 = NESEmulator.PPU.RP2C02_VSYNC * NESEmulator.PPU.RP2C02_HSYNC - NESEmulator.PPU.RP2C02_CC

-- special scanlines
NESEmulator.PPU.SCANLINE_HDUMMY = -1 -- pre-render scanline
NESEmulator.PPU.SCANLINE_VBLANK = 240 -- post-render scanline

-- special horizontal clocks
NESEmulator.PPU.HCLOCK_DUMMY = 341
NESEmulator.PPU.HCLOCK_VBLANK_0 = 681
NESEmulator.PPU.HCLOCK_VBLANK_1 = 682
NESEmulator.PPU.HCLOCK_VBLANK_2 = 684
NESEmulator.PPU.HCLOCK_BOOT = 685
NESEmulator.PPU.DUMMY_FRAME = {NESEmulator.PPU.RP2C02_HVINT / NESEmulator.PPU.RP2C02_CC - NESEmulator.PPU.HCLOCK_DUMMY, NESEmulator.PPU.RP2C02_HVINT, NESEmulator.PPU.RP2C02_HVSYNC_0}
NESEmulator.PPU.BOOT_FRAME = {
    NESEmulator.PPU.RP2C02_HVSYNCBOOT / NESEmulator.PPU.RP2C02_CC - NESEmulator.PPU.HCLOCK_BOOT,
    NESEmulator.PPU.RP2C02_HVSYNCBOOT,
    NESEmulator.PPU.RP2C02_HVSYNCBOOT
}

-- constants related to OAM (sprite)
NESEmulator.PPU.SP_PIXEL_POSITIONS = {
...
NESEmulator.PPU.TILE_LUT =
    map(
    {0x0, 0x4, 0x8, 0xc},
    function(attr)
        return NESEmulator.UTILS.transpose(
            map(
                range(0, 7),
                function(j)
                    return map(
...
local TILE_LUT,
    ...
    RP2C02_CC =
    NESEmulator.PPU.TILE_LUT,
    NESEmulator.PPU.HCLOCK_BOOT,
    NESEmulator.PPU.HCLOCK_DUMMY,
    NESEmulator.PPU.BOOT_FRAME,
    NESEmulator.PPU.DUMMY_FRAME,
    NESEmulator.CPU.FOREVER_CLOCK,
    NESEmulator.PPU.SCANLINE_VBLANK,
    NESEmulator.PPU.SCANLINE_HDUMMY,
    NESEmulator.PPU.RP2C02_CC
--------------------------------------------------------------------------------------------------------------------
-- initialization
local any_show
function NESEmulator.PPU:initialize(conf, cpu, palette)
    self.conf = conf
    self.cpu = cpu
    self.palette = palette

    self.nmt_mem = {[0] = NESEmulator.UTILS.fill({}, 0xff, 0x400, 1, -1), [1] = NESEmulator.UTILS.fill({}, 0xff, 0x400, 1, -1)}
    ...
    --self.output_pixels = {}
    self.output_pixels = NESEmulator.UTILS.fill({}, self.palette[16], NESEmulator.PPU.SCREEN_HEIGHT * NESEmulator.PPU.SCREEN_WIDTH)

    self.output_pixels_size = 0
    self.output_color = NESEmulator.UTILS.fill({}, {self.palette[1]}, 0x20) -- palette size is 0x20
    self:reset(true)
    self:setup_lut()
end

function NESEmulator.PPU:reset(mapping)
    if mapping or true then
        -- setup mapped memory
        self.cpu:add_mappings(
            range(0x2000, 0x3fff, 8),
            NESEmulator.UTILS.bind(self.peek_2xxx, self),
            NESEmulator.UTILS.bind(self.poke_2000, self)
        )
        self.cpu:add_mappings(
            range(0x2001, 0x3fff, 8),
            NESEmulator.UTILS.bind(self.peek_2xxx, self),
            NESEmulator.UTILS.bind(self.poke_2001, self)
        )
        self.cpu:add_mappings(
            range(0x2002, 0x3fff, 8),
            NESEmulator.UTILS.bind(self.peek_2002, self),
            NESEmulator.UTILS.bind(self.poke_2xxx, self)
        )
        self.cpu:add_mappings(
            range(0x2003, 0x3fff, 8),
            NESEmulator.UTILS.bind(self.peek_2xxx, self),
            NESEmulator.UTILS.bind(self.poke_2003, self)
        )
        self.cpu:add_mappings(
            range(0x2004, 0x3fff, 8),
            NESEmulator.UTILS.bind(self.peek_2004, self),
            NESEmulator.UTILS.bind(self.poke_2004, self)
        )
        self.cpu:add_mappings(
            range(0x2005, 0x3fff, 8),
            NESEmulator.UTILS.bind(self.peek_2xxx, self),
            NESEmulator.UTILS.bind(self.poke_2005, self)
        )
        self.cpu:add_mappings(
            range(0x2006, 0x3fff, 8),
            NESEmulator.UTILS.bind(self.peek_2xxx, self),
            NESEmulator.UTILS.bind(self.poke_2006, self)
        )
        self.cpu:add_mappings(
            range(0x2007, 0x3fff, 8),
            NESEmulator.UTILS.bind(self.peek_2007, self),
            NESEmulator.UTILS.bind(self.poke_2007, self)
        )
        self.cpu:add_mappings(0x3000, NESEmulator.UTILS.bind(self.peek_3000, self), NESEmulator.UTILS.bind(self.poke_2000, self))
        self.cpu:add_mappings(0x4014, NESEmulator.UTILS.bind(self.peek_4014, self), NESEmulator.UTILS.bind(self.poke_4014, self))
    end
    ...
    self.bg_pixels = NESEmulator.UTILS.fill({}, 0, 16)
    ...
    self.sp_ram = NESEmulator.UTILS.fill({}, 0xff, 0x100) -- ram size is 0x100, 0xff is a OAM garbage
    self.sp_index = 0
    self.sp_addr = 0
    self.sp_latch = 0
    ...
    self.sp_buffer = NESEmulator.UTILS.fill({}, 0, self.sp_limit)
    self.sp_buffered = 0
...
function NESEmulator.PPU:update_output_color()
    for i = 1, 0x20 do
...
function NESEmulator.PPU:setup_lut()
    self.lut_update = {}
...
function NESEmulator.PPU:set_chr_mem(mem, writable)
    self.chr_mem = mem
    self.chr_mem_writable = writable
end

NESEmulator.PPU.NMT_TABLE = {
    horizontal = {0, 0, 1, 1},
    vertical = {0, 1, 0, 1},
    four_screen = {0, 1, 2, 3},
    first = {0, 0, 0, 0},
...
function NESEmulator.PPU:nametables(mode)
    self:update(RP2C02_CC)
    local idxs = NESEmulator.PPU.NMT_TABLE[mode]
    if
        NESEmulator.UTILS.all(
            range(0, 3),
            function(i)
...
function NESEmulator.PPU:update(data_setup)
    --print "ppu update"
...
function NESEmulator.PPU:setup_frame()
    local clr = self.palette[16]
    local output = self.output_pixels
    for i = 1, NESEmulator.PPU.SCREEN_HEIGHT * NESEmulator.PPU.SCREEN_WIDTH do
        output[i] = clr
    end
    self.output_pixels_size = 0
...
function NESEmulator.PPU:vsync()
...
function NESEmulator.PPU:monitor_a12_rising_edge(monitor)
...
function NESEmulator.PPU:update_vram_addr()
...
function NESEmulator.PPU:update_scroll_address_line()
    self.name_io_addr = bor(band(bor(self.scroll_addr_0_4, self.scroll_addr_5_14), 0x0fff), 0x2000)
end

function NESEmulator.PPU:update_scroll_address_line_monitor_a12()
    self.name_io_addr = bor(band(bor(self.scroll_addr_0_4, self.scroll_addr_5_14), 0x0fff), 0x2000)
...
function NESEmulator.PPU:isactive()
    return self.scanline ~= SCANLINE_VBLANK and self.any_show
end

function NESEmulator.PPU:sync(elapsed)
...
function NESEmulator.PPU:make_sure_invariants()
...
function NESEmulator.PPU:io_latch_mask(data)
    if self:isactive() then
        return 0xff
...
-- PPUCTRL
function NESEmulator.PPU:poke_2000(_addr, data)
    self:update(RP2C02_CC)
    ...
    if self.need_nmi and self.vblank and not need_nmi_old then
        ...
        if clock < NESEmulator.PPU.RP2C02_HVINT then
...
-- PPUMASK
function NESEmulator.PPU:poke_2001(_addr, data)
    self:update(RP2C02_CC)
...
-- PPUSTATUS
function NESEmulator.PPU:peek_2002(_addr)
    self:update(RP2C02_CC)
...
-- OAMADDR
function NESEmulator.PPU:poke_2003(_addr, data)
    self:update(RP2C02_CC)
    self.regs_oam = data
    self.io_latch = data
end
...
-- OAMDATA (write)
function NESEmulator.PPU:poke_2004(_addr, data)
    self:update(RP2C02_CC)
...
-- OAMDATA (read)
function NESEmulator.PPU:peek_2004(_addr)
    if
...
-- PPUSCROLL
function NESEmulator.PPU:poke_2005(_addr, data)
    self:update(RP2C02_CC)
    self.io_latch = data
...
-- PPUADDR
function NESEmulator.PPU:poke_2006(_addr, data)
    self:update(RP2C02_CC)
    self.io_latch = data
    self.scroll_toggle = not self.scroll_toggle
    if self.scroll_toggle then
...
-- PPUDATA (write)
function NESEmulator.PPU:poke_2007(_addr, data)
...
-- PPUDATA (read)
function NESEmulator.PPU:peek_2007(_addr)
    self:update(RP2C02_CC)
...
function NESEmulator.PPU:poke_2xxx(_addr, data)
    self.io_latch = data
end

function NESEmulator.PPU:peek_2xxx(_addr)
    return self.io_latch
end

function NESEmulator.PPU:peek_3000(_addr)
    self:update(RP2C02_CC)
    return self.io_latch
end

-- OAMDMA
function NESEmulator.PPU:poke_4014(_addr, data) -- DMA
    if self.cpu:odd_clock() then
        self.cpu:steal_clocks(NESEmulator.CPU.CLK[1])
    end
    self:update(RP2C02_CC)
    self.cpu:steal_clocks(NESEmulator.CPU.CLK[1])
    data = lshift(data, 8)
    if
        self.regs_oam == 0 and data < 0x2000 and
            ((not self.any_show) or self.cpu:current_clock() <= NESEmulator.PPU.RP2C02_HVINT - NESEmulator.CPU.CLK[1] * 512)
     then
        self.cpu:steal_clocks(NESEmulator.CPU.CLK[1] * 512)
...
function NESEmulator.PPU:peek_4014(_addr)
    return 0x40
end
...
function NESEmulator.PPU:open_pattern(exp)
...
function NESEmulator.PPU:open_sprite(buffer_idx)
...
function NESEmulator.PPU:load_sprite(pat0, pat1, buffer_idx)
    local sp_buffer = self.sp_buffer
    local byte2 = sp_buffer[buffer_idx + 3]
    local pos = NESEmulator.PPU.SP_PIXEL_POSITIONS[1 + nthBitIsSetInt(byte2, 6)] -- OAM byte2 bit6: "Flip horizontally" flag
    local pat =
        bor(
...
function NESEmulator.PPU:update_address_line()
end

function NESEmulator.PPU:do_update_address_line()
    if self.a12_monitor then
...
function NESEmulator.PPU:open_name()
    if not self.any_show then
        return
    end
...
function NESEmulator.PPU:fetch_name()
    if not self.any_show then
        return
    end
...
function NESEmulator.PPU:open_attr()
    if not self.any_show then
        return
    end
...
function NESEmulator.PPU:fetch_attr()
    if not self.any_show then
        return
    end
...
function NESEmulator.PPU:fetch_bg_pattern_0()
    if not self.any_show then
        return
    end
...
function NESEmulator.PPU:fetch_bg_pattern_1()
    if not self.any_show then
        return
    end
...
function NESEmulator.PPU:scroll_clock_x()
    if not self.any_show then
        return
    end
...
function NESEmulator.PPU:scroll_reset_x()
    if not self.any_show then
        return
    end
    self.scroll_addr_0_4 = band(self.scroll_latch, 0x001f)
...
function NESEmulator.PPU:scroll_clock_y()
    if not self.any_show then
        return
    end
    if band(self.scroll_addr_5_14, 0x7000) ~= 0x7000 then
...
function NESEmulator.PPU:preload_tiles()
    if not self.any_show then
        return
    end
...
function NESEmulator.PPU:evaluate_sprites_even()
    self.sp_latch = self.sp_ram[self.sp_addr + 1]
end

function NESEmulator.PPU:evaluate_sprites_odd()
    if not self.any_show then
        return
    end
...
function NESEmulator.PPU:evaluate_sprites_odd_phase_0()
end

function NESEmulator.PPU:evaluate_sprites_odd_phase_1()
...
function NESEmulator.PPU:evaluate_sprites_odd_phase_5()
    if self.sp_latch <= self.scanline and self.scanline < self.sp_latch + self.sp_height then
...
function NESEmulator.PPU:evaluate_sprites_odd_phase_6()
    self.evaluate_sprites_odd_phase = self.evaluate_sprites_odd_phase_7
    self.sp_addr = band(self.sp_addr + 1, 0xff)
end

function NESEmulator.PPU:evaluate_sprites_odd_phase_7()
    self.evaluate_sprites_odd_phase = self.evaluate_sprites_odd_phase_8
    self.sp_addr = band(self.sp_addr + 1, 0xff)
end

function NESEmulator.PPU:evaluate_sprites_odd_phase_8()
    self.evaluate_sprites_odd_phase = self.evaluate_sprites_odd_phase_9
    self.sp_addr = band(self.sp_addr + 1, 0xff)
...
function NESEmulator.PPU:evaluate_sprites_odd_phase_9()
    self.sp_addr = band(self.sp_addr + 4, 0xff)
end

function NESEmulator.PPU:load_extended_sprites()
    if not self.any_show then
        return
    end
...
function NESEmulator.PPU:render_pixel()
...
function NESEmulator.PPU:batch_render_eight_pixels_sp()
    local pixel
...
function NESEmulator.PPU:batch_render_eight_pixels_bg()
    local pixel
    local output_color = self.output_color
    local clr = output_color[1]
    if self.bg_enabled then
...
function NESEmulator.PPU:boot()
    self.vblank = true
    self.hclk = HCLOCK_DUMMY
    self.hclk_target = FOREVER_CLOCK
end

function NESEmulator.PPU:vblank_0()
    self.vblanking = true
    self.hclk = NESEmulator.PPU.HCLOCK_VBLANK_1
end

function NESEmulator.PPU:vblank_1()
    ...
    self.batch_render_eight_pixels = self.batch_render_eight_pixels_bg
    self.hclk = NESEmulator.PPU.HCLOCK_VBLANK_2
end

function NESEmulator.PPU:vblank_2()
    if not self.vblank then
        self.vblank = self.vblanking
    end
    self.vblanking = false
...
function NESEmulator.PPU:update_enabled_flags()
    if not self.any_show then
        return
    end
...
function NESEmulator.PPU:update_enabled_flags_edge()
...
function NESEmulator.PPU:debug_logging(scanline, hclk, hclk_target)
...
function NESEmulator.PPU:run()
    if not self.fiber then
        self.fiber = coroutine.wrap(NESEmulator.UTILS.bind(self.main_loop, self), 0)
    end
...
function NESEmulator.PPU:wait_frame()
    --print "wait ppu frame"
    coroutine.yield(true)
end

function NESEmulator.PPU:wait_zero_clocks()
    if self.hclk_target <= self.hclk then
        coroutine.yield()
    end
end

function NESEmulator.PPU:wait_one_clock()
    self.hclk = self.hclk + 1
    self:wait_zero_clocks()
end

function NESEmulator.PPU:wait_two_clocks()
    self.hclk = self.hclk + 2
    self:wait_zero_clocks()
end

function NESEmulator.PPU:pre_render()
    self.vblank = false
    self.sp_overflow = false
    self.sp_zero_hit = false
    self.vblanking = false
    self.scanline = NESEmulator.PPU.SCANLINE_HDUMMY
    for i = 1, 32 do
        --341.step(589, 8) do
        -- when 341, 349, ..., 589
        self:open_name()
...
function NESEmulator.PPU:pre_render_scanline()
    -- when 320
    ...
    if self.any_show then
        self:update_enabled_flags_edge()
        if self.scanline == NESEmulator.PPU.SCANLINE_HDUMMY and self.odd_frame then
            self.cpu:set_next_frame_clock(NESEmulator.PPU.RP2C02_HVSYNC_1)
        end
    end
    self:wait_one_clock()
...
function NESEmulator.PPU:render_scanline()
    for i = 1, 32 do
        --0.step(248, 8) do
        -- when 0, 8, ..., 248
        if self.any_show then
...
function NESEmulator.PPU:post_render_scanline()
    for i = 1, 8 do
        --256.step(312, 8) do
...
function NESEmulator.PPU:post_render()
    -- when 681
    self:vblank_0()
    self:wait_zero_clocks()
...
function NESEmulator.PPU:main_loop()
    -- when 685
    ...
    while true do
        ...
        while true do
            ...
            if self.scanline ~= SCANLINE_VBLANK then
                ...
                self.hclk = NESEmulator.PPU.HCLOCK_VBLANK_0
                self:wait_zero_clocks()
                break
            end
            self:wait_zero_clocks()
