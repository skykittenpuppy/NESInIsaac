...
local DISPATCH
NESEmulator.CPU._mt = {__index = NESEmulator.CPU}

do
    NESEmulator.CPU.CLK = {}
    local clocks = {1, 2, 3, 4, 5, 6, 7, 8}
    for i = 1, #clocks do
        NESEmulator.CPU.CLK[i] = clocks[i] * NESEmulator.CPU.RP2A03_CC
    end
end
NESEmulator.CPU.UNDEFINED = {}
NESEmulator.CPU.RAM_SIZE = 0x0800
NESEmulator.CPU.MAINMEM_SIZE = 0x10000
NESEmulator.CPU.NMI_VECTOR = 0xfffa
NESEmulator.CPU.RESET_VECTOR = 0xfffc
NESEmulator.CPU.IRQ_VECTOR = 0xfffe

NESEmulator.CPU.IRQ_EXT = 0x01
NESEmulator.CPU.IRQ_FRAME = 0x40
NESEmulator.CPU.IRQ_DMC = 0x80

NESEmulator.CPU.ClockRates = {
    Ntsc = 1789773,
    Pal = 1662607,
    Dendy = 1773448
}
local RAM_SIZE, UNDEFINED, NMI_VECTOR, IRQ_VECTOR, FOREVER_CLOCK =
    NESEmulator.CPU.RAM_SIZE,
    NESEmulator.CPU.UNDEFINED,
    NESEmulator.CPU.NMI_VECTOR,
    NESEmulator.CPU.IRQ_VECTOR,
    NESEmulator.CPU.FOREVER_CLOCK

local UNDEFINED = NESEmulator.CPU.UNDEFINED
local CLK = NESEmulator.CPU.CLK

local isDefined = NESEmulator.UTILS.isDefined
local bind = NESEmulator.UTILS.bind
local tSetter = NESEmulator.UTILS.tSetter
local tGetter = NESEmulator.UTILS.tGetter
local fill = NESEmulator.UTILS.fill
local range = NESEmulator.UTILS.range
local printf = NESEmulator.UTILS.printf
local nthBitIsSetInt = NESEmulator.UTILS.nthBitIsSetInt

NESEmulator.CPU.PokeNop = function()
end

function NESEmulator.CPU:steal_clocks(clk)
    self.clk = self.clk + clk
end

function NESEmulator.CPU:odd_clock()
    return ((self.clk_total + self.clk) % CLK[2]) ~= 0
end

function NESEmulator.CPU:update()
    self.apu:clock_dma(self.clk)
    return self.clk
end

function NESEmulator.CPU:dmc_dma(addr)
...
function NESEmulator.CPU:next_frame_clock()
    return self.clk_frame
end

function NESEmulator.CPU:set_next_frame_clock(x)
    self.clk_frame = x
    self.clk_target = x < self.clk_target and x or self.clk_target
    return x
end
function NESEmulator.CPU:current_clock()
    return self.clk
end

function NESEmulator.CPU:peek_nop(addr)
    return rshift(addr, 8)
end

function NESEmulator.CPU:peek_jam_1(addr)
    self._pc = band(self._pc - 1, 0xffff)
    return 0xfc
end
function NESEmulator.CPU:peek_jam_2(_addr)
    return 0xff
end

function NESEmulator.CPU:peek_ram(addr)
    return self.ram[addr % CPRAM_SIZE]
end
function NESEmulator.CPU:poke_ram(addr, data)
    self.ram[addr % RAM_SIZE] = data
end

-- read an addr (8 bit)
function NESEmulator.CPU:fetch(addr)
    return self._fetch[addr](addr)
end
function NESEmulator.CPU:store(addr, value)
    return self._store[addr](addr, value)
end

-- Read 16 bits (word) from addr
function NESEmulator.CPU:peek16(addr)
    return self:fetch(addr) + lshift(self:fetch(addr + 1), 8)
end

function NESEmulator.CPU:add_mappings(addr, peek, poke)
    ...
    for i = addr[0] and 0 or 1, #addr do
        ...
        if poke and type(poke) ~= "table" then
            ...
            self._store[x] = NESEmulator.CPU.PokeNop
        end
    end
end

function NESEmulator.CPU:reset()
    self._a = 0
    self._x = 0
    self._y = 0
    self._sp = 0xfd
...
function NESEmulator.CPU:new(conf)
    local cpu = {}
    setmetatable(cpu, NESEmulator.CPU._mt)
    cpu.conf = conf or {loglevel = 0, pc = nil}
    cpu.ram = fill({}, UNDEFINED, RAM_SIZE)
    cpu._store = fill({}, UNDEFINED, NESEmulator.CPU.MAINMEM_SIZE)
    cpu._fetch = fill({}, UNDEFINED, NESEmulator.CPU.MAINMEM_SIZE)
    cpu.peeks = {}
    cpu.pokes = {}
    cpu.clk_rate = NESEmulator.CPU.ClockRates.Ntsc
...
function NESEmulator.CPU:sprite_dma(addr, sp_ram)
    do
        local ram = self.ram
        for i = 1, 256 do
...
function NESEmulator.CPU:boot()
    self.clk = CLK[7] -- clocks it takes to boot
    self._pc = self.conf.pc or self:peek16(NESEmulator.CPU.RESET_VECTOR)
end

function NESEmulator.CPU:vsync()
...
function NESEmulator.CPU:clear_irq(line)
    local old_irq_flags = band(self.irq_flags, bor(NESEmulator.CPU.IRQ_FRAME, NESEmulator.CPU.IRQ_DMC))
    self.irq_flags = band(bxor(self.irq_flags, bxor(line, bor(bor(NESEmulator.CPU.IRQ_EXT, NESEmulator.CPU.IRQ_FRAME), NESEmulator.CPU.IRQ_DMC))))
...
function NESEmulator.CPU:next_interrupt_clock(clk)
...
function NESEmulator.CPU:do_irq(line, clk)
...
function NESEmulator.CPU:do_nmi(clk)
    if self.clk_nmi == FOREVER_CLOCK then
        self.clk_nmi = self:next_interrupt_clock(clk)
    end
end

function NESEmulator.CPU:do_isr(vector)
    if self.jammed then
        return
    end
...
function NESEmulator.CPU:fetch_irq_isr_vector()
...
function NESEmulator.CPU:flags_pack()
    local nz = self._p_nz
    return bor(
        bor(
            bor(
...
function NESEmulator.CPU:flags_unpack(f)
...
------ branch helper ------
function NESEmulator.CPU:branch(cond)
    if cond then
...
function NESEmulator.CPU:imm(_read, _write)
...
function NESEmulator.CPU:zpg(read, write)
...
function NESEmulator.CPU:zpg_reg(indexed, read, write)
...
function NESEmulator.CPU:zpg_x(read, write)
    return self:zpg_reg(self._x, read, write)
end

function NESEmulator.CPU:zpg_y(read, write)
    return self:zpg_reg(self._y, read, write)
end
-- absolute addressing
function NESEmulator.CPU:abs(read, write)
...
function NESEmulator.CPU:abs_reg(indexed, read, write)
...
function NESEmulator.CPU:abs_x(read, write)
    return self:abs_reg(self._x, read, write)
end

function NESEmulator.CPU:abs_y(read, write)
    return self:abs_reg(self._y, read, write)
end
-- indexed indirect addressing
function NESEmulator.CPU:ind_x(read, write)
...
function NESEmulator.CPU:ind_y(read, write)
...
function NESEmulator.CPU:read_write(read, write)
    if read then
...
-- load instructions
function NESEmulator.CPU:_lda()
    self._p_nz = self.data
    self._a = self.data
end

function NESEmulator.CPU:_ldx()
    self._p_nz = self.data
    self._x = self.data
end

function NESEmulator.CPU:_ldy()
    self._y = self.data
    self._p_nz = self.data
end

-- store instructions
function NESEmulator.CPU:_sta()
    self.data = self._a
end

function NESEmulator.CPU:_stx()
    self.data = self._x
end

function NESEmulator.CPU:_sty()
    self.data = self._y
end

-- transfer instructions
function NESEmulator.CPU:_tax()
    self.clk = self.clk + CLK[2]
    local a = self._a
    self._x = a
    self._p_nz = a
...
function NESEmulator.CPU:_jmp_a()
    self._pc = self:peek16(self._pc)
    self.clk = self.clk + CLK[3]
end

function NESEmulator.CPU:_jmp_i()
...
function NESEmulator.CPU:_rts()
    local x = self:pull16()
    self._pc = band(x + 1, 0xffff)
    self.clk = self.clk + CLK[6]
end

function NESEmulator.CPU:_rti()
    self.clk = self.clk + CLK[6]
...
function NESEmulator.CPU:_bne()
    return self:branch(band(self._p_nz, 0xff) ~= 0)
end

function NESEmulator.CPU:_beq()
    return self:branch(band(self._p_nz, 0xff) == 0)
end

function NESEmulator.CPU:_bmi()
    return self:branch(band(self._p_nz, 0x180) ~= 0)
end

function NESEmulator.CPU:_bpl()
    return self:branch(band(self._p_nz, 0x180) == 0)
end

function NESEmulator.CPU:_bcs()
    return self:branch(self._p_c ~= 0)
end

function NESEmulator.CPU:_bcc()
    return self:branch(self._p_c == 0)
end

function NESEmulator.CPU:_bvs()
    return self:branch(self._p_v ~= 0)
end

function NESEmulator.CPU:_bvc()
    return self:branch(self._p_v == 0)
end

-- math operations
function NESEmulator.CPU:_adc()
...
function NESEmulator.CPU:_dec()
    self._p_nz = band(self.data - 1, 0xff)
    self.data = self._p_nz
end

function NESEmulator.CPU:_inc()
    self._p_nz = band(self.data + 1, 0xff)
    self.data = self._p_nz
end

function NESEmulator.CPU:_dex()
    self.clk = self.clk + CLK[2]
...
-- flags instructions
function NESEmulator.CPU:_clc()
    self.clk = self.clk + CLK[2]
    self._p_c = 0
end

function NESEmulator.CPU:_sec()
    self.clk = self.clk + CLK[2]
    self._p_c = 1
end

function NESEmulator.CPU:_cld()
    self.clk = CLK[2] + self.clk
    self._p_d = 0
end

function NESEmulator.CPU:_sed()
    self.clk = self.clk + CLK[2]
    self._p_d = 8
end

function NESEmulator.CPU:_clv()
    self.clk = self.clk + CLK[2]
    self._p_v = 0
end
function NESEmulator.CPU:_sei()
    self.clk = self.clk + CLK[2]
    if self._p_i == 0 then
        self._p_i = 0x04
        self.clk_irq = FOREVER_CLOCK
...
-- stack operations
function NESEmulator.CPU:_pha()
    self.clk = self.clk + CLK[3]
    return self:push8(self._a)
end

function NESEmulator.CPU:_php()
    self.clk = self.clk + CLK[3]
    local data = bor(self:flags_pack(), 0x10)
    return self:push8(data)
end

function NESEmulator.CPU:_pla()
    self.clk = self.clk + CLK[4]
    self._a = self:pull8()
    self._p_nz = self._a
end

function NESEmulator.CPU:_plp()
    self.clk = self.clk + CLK[4]
    local i = self._p_i
...
function NESEmulator.CPU:_tsx()
    self.clk = self.clk + CLK[2]
    self._p_nz = self._sp
    self._x = self._sp
end

function NESEmulator.CPU:_txs()
    self.clk = self.clk + CLK[2]
    self._sp = self._x
end

-- undocumented instructions, rarely used
function NESEmulator.CPU:_anc()
    self._a = band(self._a, self.data)
    self._p_nz = self._a
    self._p_c = rshift(self._p_nz, 7)
end

function NESEmulator.CPU:_ane()
    self._a = band(band(bor(self._a, 0xee), self._x), self.data)
    self._p_nz = self._a
end

function NESEmulator.CPU:_arr()
...
function NESEmulator.CPU:_asr()
    self._p_c = band(band(self.data, self._a), 0x1)
    self._a = rshift(band(self.data, self._a), 1)
    self._p_nz = self._a
end

function NESEmulator.CPU:_dcp()
    self.data = band(self.data - 1, 0xff)
    return self:_cmp()
end

function NESEmulator.CPU:_isb()
    self.data = band(self.data + 1, 0xff)
    return self:_sbc()
end

function NESEmulator.CPU:_las()
...
function NESEmulator.CPU:_lax()
    self._x = self.data
    self._p_nz = self._x
    self._a = self._x
end

function NESEmulator.CPU:_lxa()
    self._x = self.data
    self._p_nz = self._x
    self._a = self._x
end

function NESEmulator.CPU:_rla()
    local c = self._p_c
    self._p_c = rshift(self.data, 7)
...
function NESEmulator.CPU:_sax()
    self.data = band(self._a, self._x)
end

function NESEmulator.CPU:_sbx()
...
-- nops
function NESEmulator.CPU:_nop()
end

-- interrupts
function NESEmulator.CPU:_brk()
    local data = self._pc + 1
    self:push16(data)
...
--------------------------------------------------------------------------------------------------------------------
-- default core
function NESEmulator.CPU:printState(doFetch)
    --[
    local ppuclk = 0
    if self.ppu then
...
function NESEmulator.CPU:r_op(instr, mode)
    self[mode](self, true, false)
    self[instr](self)
end

function NESEmulator.CPU:w_op(instr, mode, store)
    self[mode](self, false, true)
    self[instr](self)
    self[store](self)
end

function NESEmulator.CPU:rw_op(instr, mode, store)
    self[mode](self, true, true)
    self[instr](self)
    self[store](self)
end

function NESEmulator.CPU:a_op(instr)
    self.clk = self.clk + CLK[2]
    self.data = self._a
    self[instr](self)
    self._a = self.data
...
function NESEmulator.CPU:no_op(_instr, ops, ticks)
    self._pc = self._pc + ops
    self.clk = self.clk + ticks * NESEmulator.CPU.RP2A03_CC
end

function NESEmulator.CPU:do_clock()
...
local asd = 0
function NESEmulator.CPU:run_once()
...
NESEmulator.CPU.ADDRESSING_MODES = {
...
NESEmulator.CPU.DISPATCH = {}
DISPATCH = NESEmulator.CPU.DISPATCH
    ...
    for _, opcode in ipairs(opcodes) do
        ...
        if type(args) == "table" then
            if (args[1] == "r_op" or args[1] == "w_op" or args[1] == "rw_op") then
                ...
                addrmode = NESEmulator.CPU.ADDRESSING_MODES[addrmode][band(rshift(opcode, 2), 7) + 1]
