local serpent = include("NES/libs/serpent")
...
local map, rotatePositiveIdx, nthBitIsSet, nthBitIsSetInt, range, bind =
    ...
    NESEmulator.UTILS.bind

NESEmulator.ROM = {}
NESEmulator.ROM._mt = {__index = NESEmulator.ROM}
...
NESEmulator.ROM.MAPPER_DB = {
    [0x00] = NESEmulator.ROM
}

function NESEmulator.ROM:initialize(conf, cpu, ppu, basename, bytes, str)
    self.conf = conf or {}
    self.cpu = cpu
    self.ppu =
        ppu or
    ...
    do
        ...
        for i = 1, prg_count do
            self.prg_banks[i] = NESEmulator.UTILS.copy(bytes, prg_bank_size, idx)
    ...
    do
        ...
        for i = 1, chr_count do
            self.chr_banks[i] = NESEmulator.UTILS.copy(bytes, chr_bank_size, idx)
            idx = idx + chr_bank_size
        end
    end
    self.prg_ref = {}
    NESEmulator.UTILS.fill(self.prg_ref, 0, 0x10000)
    ...
    self.chr_ref = self.chr_ram and (NESEmulator.UTILS.fill({}, 0, 0x2000)) or NESEmulator.UTILS.copy(self.chr_banks[1])
...
function NESEmulator.ROM:init()
end

function NESEmulator.ROM:reset()
    self.cpu:add_mappings(range(0x8000, 0xffff), NESEmulator.UTILS.tGetter(self.prg_ref), NESEmulator.CPU.UNDEFINED)
end

function NESEmulator.ROM:peek_6000(addr)
    return self.wrk_readable and self.wrk[addr - 0x6000] or rshift(addr, 8)
end

function NESEmulator.ROM:poke_6000(addr, data)
    if self.wrk_writable then
        self.wrk[addr - 0x6000] = data
    end
end

function NESEmulator.ROM:vsync()
end

function NESEmulator.ROM:load_battery()
...
function NESEmulator.ROM:save_battery()
    if not self.battery then
        return
    end
    local sav = self.basename + ".sav"
...
function NESEmulator.ROM:new(conf, cpu, ppu, basename, bytes, str)
    local rom = {}
    setmetatable(rom, NESEmulator.ROM._mt)
    rom:initialize(conf, cpu, ppu, basename, bytes, str)
    return rom
end

function NESEmulator.ROM.load(conf, cpu, ppu)
...
function UxROM:new(...)
    local rom = NESEmulator.ROM:new(table.unpack(...))
    setmetatable(rom, UxROM._mt)
    return rom
end
function UxROM:reset()
    self.cpu:add_mappings(range(0x8000, 0xffff), NESEmulator.UTILS.tGetter(self.prg_ref), bind(self.poke_8000, self))
end
...
NESEmulator.ROM.MAPPER_DB[0x02] = UxROM

local CNROM = {}
CNROM._mt = {__index = CNROM}
setmetatable(CNROM, {__index = NESEmulator.ROM})
function CNROM:new(...)
    local rom = NESEmulator.ROM:new(table.unpack(...))
...
function CNROM:reset()
    self.cpu:add_mappings(
        range(0x8000, 0xffff),
        NESEmulator.UTILS.tGetter(self.prg_ref),
        self.chr_ram and bind(self.poke_8000, self) or NESEmulator.CPU.UNDEFINED
    )
end

function CNROM:poke_8000(_addr, data)
    local j = band(data, 3)
    self.chr_ref = {table.unpack(self.chr_banks[j])}
end
NESEmulator.ROM.MAPPER_DB[0x03] = CNROM

local MMC1 = {}
MMC1._mt = {__index = MMC1}
setmetatable(MMC1, {__index = NESEmulator.ROM})
function MMC1:new(...)
    local rom = {}
...
function MMC1:reset()
    ...
    local old = NESEmulator.UTILS.copy(self.chr_banks)
    for i = 2, #chr do
        chr[i] = nil
    end
    chr[1] = {}
    ...
    self.cpu:add_mappings(range(0x8000, 0xffff), NESEmulator.UTILS.tGetter(self.prg_ref), bind(self.poke_prg, self))
...
NESEmulator.ROM.MAPPER_DB[0x01] = MMC1

local MMC3 = {}
MMC3._mt = {__index = MMC3}
setmetatable(MMC3, {__index = NESEmulator.ROM})
function MMC3:new(...)
    local rom = NESEmulator.ROM:new(table.unpack(...))
...
function MMC3:init(rev) -- rev = :A or :B or :C
    ...
    self.chr_bank_mapping = NESEmulator.UTILS.fill({}, nil, 8)
...
function MMC3:reset()
    ...
    local poke_a000 = self.mirroring ~= "FourScreen" and bind(self.poke_a000, self) or NESEmulator.CPU.UNDEFINED
    self.cpu:add_mappings(range(0x6000, 0x7fff), bind(self.peek_6000, self), bind(self.poke_6000, self))
    local g = NESEmulator.UTILS.tGetter(self.prg_ref)
    ...
    self.clock = 0
    if NESEmulator.PPU then
        self.hold = NESEmulator.PPU.RP2C02_CC * 16
    end
...
function MMC3:poke_8000(_addr, data)
    ...
    if chr_bank_swap ~= self.chr_bank_swap then
        ...
        if not self.chr_ram then
            self.ppu.update(0)
            self.chr_ref = NESEmulator.UTILS.rotate(self.chr_ref, 0x1000)
            self.chr_bank_mapping = NESEmulator.UTILS.rotate(self.chr_bank_mapping, 4)
        end
    end
end
...
function MMC3:poke_e000(_addr, _data)
    self.ppu:update(0)
    self.enabled = false
    self.cpu:clear_irq(NESEmulator.CPU.IRQ_EXT)
end
...
function MMC3:a12_signaled(cycle)
    ...
    if flag and self.count == 0 and self.enabled then
        self.cpu:do_irq(NESEmulator.CPU.IRQ_EXT, cycle)
    end
end

NESEmulator.ROM.MAPPER_DB[0x04] = MMC3
